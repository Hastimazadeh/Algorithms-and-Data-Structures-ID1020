...
//this class is exactly the same as the symbol graph used for task 1 and 2 but instead it uses Digraph class
...
public class SymbolDigraph_Task3 {
	...
	private Digraph_Task3 diigraph; // the graph
	...
	public SymbolDigraph_Task3 (File file, String delimiter) throws FileNotFoundException
	{
		...
		//??? how is this working with the constructor in Graph(Scanner in)
		diigraph = new Digraph_Task3(st.size()); //public Graph_Task1(int V)  //setting the size of the graph  to st.size
		in = new Scanner(file); 
		...
		while (in.hasNextLine())  // as long as it has a new line
		{
			...
			{
				int w = st.get(a[i]);
				diigraph.addEdge(v, w);  //public void addEdge(int v, int w)
			}
		}
	}
	...
	public Digraph_Task3 diiigraph() { 
		return diigraph; 
	} 
