...
// its the same as DFS for task1 but instead it uses digraph

package task1;


public class DirectedDFS_Task3 {
	...
	public DirectedDFS_Task3 (Digraph_Task3 G, int s) // initialises data structure
	{
	...
	//Breadth-first search from a single source //This method is called depth-first search (DFS). 
	//To search a graph, invoke a recursive method that visits vertices. To visit a vertex:

	private void dfs(Digraph_Task3 G, int v) 
	{
		...
		for (int w : G.adj(v))//B) Visit (recursively) all the vertices that are adjacent to it
			if (!visited[w])//B) and that have not yet been marked
			{
				...
				//we remember the edge v-w that takes us to each vertex w for the first time, by setting edgeTo[w] to v. 
				//In other words, v-w is the last edge on the known path from s to w.
